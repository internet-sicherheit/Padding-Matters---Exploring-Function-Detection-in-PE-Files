...
def process_binary(binary_path: Path):
    ...
    ends_pred = torch.where(labels == 1)[0].tolist()

    # translate starts and ends to pairs of boundaries
    boundaries = translate_predictions(starts_pred, ends_pred)

    output_file = OUTPUT_DICT / f'{sha256}.out'
    output_strs = []
    for bound in boundaries:
        output_strs.append(f'{bound[0]}, {bound[1]}')
