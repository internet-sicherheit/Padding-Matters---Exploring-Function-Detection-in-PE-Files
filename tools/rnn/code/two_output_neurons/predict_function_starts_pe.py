...
def mark_function_starts(section: Dict[int, Tuple[int, int]], function_starts: np.ndarray):
    ...
    function_start_indices = np.where(function_starts == 1)[0]
...
def map_probabilities(section: Dict[int, Tuple[int, int]], probabilities: np.ndarray) -> List[Tuple[int, float, float]]:
    """
    Takes the class probabilities for each byte index in the given sections and maps them to the corresponding bytes.

    :param section: memory mapped representation of the bytes within the section
    :param probabilities: an array of slices that hold the class probability of each byte
    :return: list of tuples that contain the VA, probability for function start class and probability for no function
     start class
    """
    ...
    for byte_slice in probabilities:
        for probability in byte_slice:
            function_start_true = probability[0]
            function_start_false = probability[1]
            # We do not need very precise probabilities. Therefore, we can round the probability.
            function_start_true = round(float(function_start_true), 5)
            function_start_false = round(float(function_start_false), 5)
            ...
            try:
                ...
                break
            mapped_probabilities.append((current_va, function_start_true, function_start_false))

    return mapped_probabilities


def predict_function_starts(pe: pefile.PE, model: Sequential, include_probabilities: bool = False) \
        -> Union[List[Tuple[int, int]], Tuple[List[Tuple[int, int]], List[Tuple[int, float, float]]]]:
    """
    ...
    for section in memory_map:
        ...
        if include_probabilities:
            probabilities += map_probabilities(section, prediction)
        # get the predicted class depending on the predicted class probabilities
        results = np.argmax(prediction, axis=2)
        mark_function_starts(section, results)
...
def main():
    ...
    # predict function starts and print the results
